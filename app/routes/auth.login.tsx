...
import type { ActionFunction, LinksFunction, LoaderFunction } from '@remix-run/cloudflare';
...
import SuperTokensHelpers, { isAuthenticated } from '~/utils/supertokens/index.server';
  ...
  return (
    <main>
      <Form ref={useMemo(autoObserve, [autoObserve])} method="post" onSubmit={handleSubmit}>
        ...
        <input
          ...
          {...configure('email', {
            ...
            async validate({ value }: HTMLInputElement) {
              ...
              const response = await fetch(`/auth/email-exists?email=${value}`);
        ...
        {mode === 'signin' && (
          <Link className="forgot-password" to={routes.resetPassword}>
            Forgot password?
          </Link>
        )}
      </Form>
...
export const loader: LoaderFunction = async ({ request, response }) => {
  if (await isAuthenticated({ request, response })) return redirect('/', 303);
...
export const action: ActionFunction = async ({ request, response }) => {
  ...
  const { email, password, mode } = await request.formData().then(Object.fromEntries);
