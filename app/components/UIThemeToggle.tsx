...
export function UIThemeLoader() {
  ...
  useEffect(() => {
    if (uiTheme.value === null) {
      uiTheme.value = matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light';
    }
  }, []);
  ...
  useSignalEffect(() => {
    if (uiTheme.value === 'dark') {
      ...
      persistTheme.submit({ theme: uiTheme.value }, { action: '/', method: 'post' });
    } else {
...
export function UIThemeToggle() {
  return (
    <button
      ...
      onClick={() => (uiTheme.value = uiTheme.value === 'dark' ? 'light' : 'dark')}
    >
      {uiTheme.value === 'light' ? (
