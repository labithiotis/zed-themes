...
export const uiTheme = signal<UITheme>('light');

export function UIThemeToggle() {
  const { submit } = useFetcher();

  const toggle = () => {
    if (uiTheme.value === 'light') {
      document.body.classList.add('dark');
      submit({ theme: 'dark' }, { action: '/action/theme', method: 'post' });
    } else {
      document.body.classList.remove('dark');
      submit({ theme: 'light' }, { action: '/action/theme', method: 'post' });
    }
  };
